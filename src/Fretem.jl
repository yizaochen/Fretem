module Fretem
using LinearAlgebra, Statistics, SparseArrays, Arpack

    export lglnodes, getLagrange, getLagrange_with_xref, get_fem_xref_weights_basis, fem_solve_eigen, fem_solve_eigen_by_pref
    ...
    """
    fem_solve_eigen(Nh, Np, xratio, xavg, Vref, D_array, Nv)

    Using spectral finite element method to diagonalize the H operator
    """
    function fem_solve_eigen(Nh, Np, xratio, xavg, Vref, D, Nv)
        N, xref, w0, Ldx, w = get_fem_xref_weights_basis(Nh, Np, xratio, xavg)
        pref = get_pref(Vref, w0)
